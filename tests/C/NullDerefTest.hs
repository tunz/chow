...
cNullDerefTests = testGroup "null-deref tests for C" [systemTests, unitTests]

systemTests =
  testGroup
    "null-deref system tests for C"
    ...
    , testCase
        "null-deref: ptr deref in decl and check it in nested if (case 3)" $
  ...
  --   >>= assertEqual "" [ newPos "examples/c/nullptr6.c" 16 23 ] . map fst
    ...
    , testCase "null-deref: ternary conditional expr (case 10)" $
      runChecker
        (Options TypeNullDeref LangC ["examples/c/nullptr/nullptr10.c"]) >>=
    ...
    , testCase "null-deref: FPs by mishandling of terminated paths (case 11)" $
      runChecker
        (Options TypeNullDeref LangC ["examples/c/nullptr/nullptr11.c"]) >>=
      assertEqual "" [] . map fst
    , testCase
        "null-deref: nested access, but only null-dref-check of root (case 12)" $
      runChecker
        (Options TypeNullDeref LangC ["examples/c/nullptr/nullptr12.c"]) >>=
      assertEqual "" [newPos "examples/c/nullptr/nullptr12.c" 13 18] . map fst
    , testCase "null-deref: ptr access in if condition (case 13)" $
      runChecker
        (Options TypeNullDeref LangC ["examples/c/nullptr/nullptr13.c"]) >>=
    ...
    , testCase "null-deref: FPs by assumptions of a condition (case 14)" $
      runChecker
        (Options TypeNullDeref LangC ["examples/c/nullptr/nullptr14.c"]) >>=
      assertEqual "" [] . map fst
    ]
...
genFAsTC expr b result =
  ...
  assertEqual
    ""
    (Map.fromList (fillDummyPos result))
    (callFindAssumptions b expr)
  where
    fillDummyPos = List.map (\(name, stat) -> ((name, dummyPos), stat))
    dummyPos = newPos "" 0 0

unitTests =
  testGroup
    "null-deref findAssumptions tests for C"
    ...
    , genFAsTC "!ptr || !ptr2" False [("ptr", IsNotNull), ("ptr2", IsNotNull)]
    , genFAsTC "(!ptr && !ptr2)" True [("ptr", IsNull), ("ptr2", IsNull)]
    , genFAsTC "(ptr && !ptr2)" False []
    , genFAsTC "(ptr && !ptr2) || ptr3" True []
    , genFAsTC "(ptr && !ptr2) || ptr3" False [("ptr3", IsNull)]
    , genFAsTC "(ptr || !ptr2) && ptr3" True [("ptr3", IsNotNull)]
