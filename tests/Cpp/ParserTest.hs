{-# LANGUAGE LambdaCase #-}
module Cpp.ParserTest (cppParserTests) where
...
parseAndAssert name parser stream isAns =
  case parse parser "" (tokenize stream) of
    Right result | isAns result -> return ()
    Right result -> assertFailure $ name ++ " fail\ngot: " ++ show result
    Left err -> assertFailure $ name ++ " fail\ngot err: " ++ show err

cppParserTests = testGroup "Parser tests for C++"
  [ testCase "if if else else if else" $
    parseAndAssert "blockParser" blockParser
                   ("if (ptr) " ++
                   "if (ptr2) return; else return;" ++
                   "else if (!ptr) return; else return;")
                   (\case
                      (CompoundStmt [IfStmt _ _
                                     (CompoundStmt [IfStmt _ _ _ (Just _)])
                                     (Just (IfStmt _ _ _ (Just _)))]) -> True
                      _ -> False
                    )
  , testCase "Some::Function () { }" $
    parseAndAssert "functionParser" functionParser
  ...
  , testCase "for (auto &x: xs) { }" $
    parseAndAssert "forRangeParser" forRangeParser
  ...
  , testCase "fn(fn2(a1, a2), a3)" $
    parseAndAssert "exprParser" exprParser
                   "fn(fn2(a1, a2), a3)"
                   (\case
...
typeParserTests = testGroup "Type parser tests for C++"
  [ testCase "int" $
    parseAndAssert "typeDeclParser" typeDeclParser
                   "int"
                   (== IntType)
  ...
  , testCase "std::map<int, ReadbackRequestCallback>::iterator it" $
    parseAndAssert "declParser" declParser
