...
import Data.Map as Map (empty, insert, Map, unionWith, (!), (\\),
                        intersectionWith, lookup, unionsWith,
                        elemAt, null, delete, union, insertWith, filter)
...
--        : 0);
data PtrState = Deref | IsNull | IsNotNull | IsUnknown | IsBoth
  deriving (Eq, Show)
type VarName = String
...
data CheckResult = Update (VarMap, PtrSet)
                 | Continue PtrSet
                 | Found Report
                 | Stop
                 | Reset
...
newReport var notNullPos belief2 pos2 =
  Found (notNullPos,
         "[Detect]\n"
         ++ var ++ " is dereferenced" ++ " in " ++ show notNullPos ++ "\n"
         ++ var ++ " is believed to be '" ++ show belief2 ++
           "' in " ++ show pos2 )
    ...
    checkPtrAccess' expr ptrSet =
      case expr of
        ...
        BinOp And expr1 expr2 ->
          inspectAndMerge' expr1 (Just expr2, Nothing)
        BinOp Or expr1 expr2 ->
          inspectAndMerge' expr1 (Nothing, Just expr2)
      ...
      where
        ...
        searchRHSs expr =
          case expr of
            Comma expr1 expr2 ->
              searchRHSs expr1 |>> searchRHSs expr2
        ...
        insertAllPtrs expr =
          case expr of
            ...
            var@(ArrayAccess pos expr' _) ->
              trySetDeref (getVarInfo' (show var)) pos
              |>> insertAllPtrs expr'
...
isCheckingNullPtr varMap ptrSet = isCheckingNullPtr'
  where
    isCheckingNullPtr' cond =
      case cond of
        BinOp op cond' (Var _ "NULL") | op == Eq || op == Neq ->
          isCheckingNullPtr' cond'
        BinOp op (Var _ "NULL") cond' | op == Eq || op == Neq ->
          isCheckingNullPtr' cond'
        Var pos v | isDeref v -> newReport' v pos
        v@(ArrayAccess pos _ _) | isDeref (show v) -> newReport' (show v) pos
        v@(MemberPtrAccess p _ _) | isDeref (show v) -> newReport' (show v) p
        v@(MemberAccess pos _ _) | isDeref (show v) -> newReport' (show v) pos
        Comma _ x -> isCheckingNullPtr' x
        UnOp Not x -> isCheckingNullPtr' x
        BinOp op x y | op == And || op == Or ->
          isCheckingNullPtr' x |>> isCheckingNullPtr' y
        TernaryCond cond' expr1 expr2 -> isCheckingNullPtr' cond'
        _ -> Continue emptyPtrSet
      where newReport' v = newReport v (snd (ptrSet ! getVarInfo v varMap)) IsBoth
            isDeref v =
              case Map.lookup (getVarInfo v varMap) ptrSet of
                Just (stat, _) -> stat == Deref
                _ -> False

inspectAndMerge :: Show a
             => VarMap
             -> PtrSet
             -> Expr
             -> (Maybe a, Maybe a)
...
inspectAndMerge varMap ptrSet cond (thenbr, elsebr) runFn =
  isCheckingNullPtr varMap ptrSet cond
  |>> checkPtrAccess cond varMap ptrSet |>>= \ptrSet' ->
     unionCheckResult (runIfJust (thencond `union` ptrSet') thenbr)
  ...
  where
    ...
    mergePtrs initPtrSet newPtrSets = allPtrs \\ removedPtrs
      where allPtrs = unionsPtrSet (initPtrSet' : newPtrSets')
...
checkNullDeref isSwitch varMap ptrSet stmt =
  case stmt of
    ...
    IfStmt _ cond thbr elbr ->
      inspectAndMerge varMap ptrSet cond (Just thbr, elbr)
