...
data Expr
  ...
  | TernaryCond Expr
                Expr
                Expr
  | ArrayAccess SourcePos
                Expr
                Expr
  | MemberAccess SourcePos
                 Expr
                 Expr
  | Comma Expr
          Expr
...
prettyShowStmt depth (ForStmt pos init cond inc block) =
  ...
  show init ++
  "; " ++ show cond ++ "; " ++ show inc ++ ")\n" ++ prettyShowStmt depth block
prettyShowStmt depth (ForRangeStmt pos decl iterable block) =
  "for (" ++
  show decl ++ " : " ++ show iterable ++ ")\n" ++ prettyShowStmt depth block
